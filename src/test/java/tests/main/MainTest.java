...
public class MainTest extends BaseTest {
    ...
    public void registerNowButtonClick () {
        basePage.closePopup();
        int goodTitlesCount = 0;
        for (int i = 1; i <= NUMBER_OF_REGISTER_NOW_BUTTON_MAIN_PAGE; i++) {
            basePage.waitOneSeconds();
            mainPage.registerNowButtonClick(i);
            basePage.nextTab();
        ...
        assertEquals(NUMBER_OF_REGISTER_NOW_BUTTON_MAIN_PAGE, goodTitlesCount);
    }

    @Test(invocationCount = 2, successPercentage = 99)
    public void slotButtonTest () {
        basePage.closePopup();
        int goodTitlesCount = 0;
        for (int i = 1; i <= NUMBER_OF_SLOT_BUTTON_MAIN_PAGE; i++) {
            basePage.waitOneSeconds();
            mainPage.slotButtonClick(i);
            basePage.waitOneSeconds();
            if (basePage.checkTitles(REVIEW_TITLES)) { goodTitlesCount++; }
            else { basePage.outputWrongTitle(); }
            basePage.back(MAIN_PAGE_URL);
        }
        assertEquals(NUMBER_OF_SLOT_BUTTON_MAIN_PAGE, goodTitlesCount);
    }
